function addHoverTag(targetLanguage) {
    // (?![^<]*?>)([A-z0-9']+)
    // - (?![^<]*?>) to not consider any html tags. or rather, to not consider anything wrapped in <>. But since all <'s are coded as &lt; in html, it only affects tags
    // - ([A-z0-9']+) apart from tags, we consider all words, numbers or words containing "'"
    ...
    $("hover").hover(function() {
        ...
        setTimeoutConst = setTimeout(async function () {
            ...
            if(self.text() === text) {
                let translation = await translate(text, 'eng', targetLanguage);
